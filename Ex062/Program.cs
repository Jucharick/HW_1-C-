...
int[,] matrixSpiral1 = BySpiral(matrix);
PrintMatrix(matrixSpiral1);
...
int[,] BySpiral(int[,] matr)
{
    int temp = 1; // значение, которое мы вставляем в ячейку в матрице
    int i = 0;
    int j = 0;
    // два индекса, по которым мы обращаемся к матрице.
